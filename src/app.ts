...
import { PriceChartingApiProvider } from './provider/pricecharting.js';
...
export interface CreateApplicationOptions {
  readonly config?: AppConfig;
  readonly logger?: Logger;
  readonly provider?: PriceChartingProvider;
}
...
export const createApplication = (options: CreateApplicationOptions = {}): Application => {
  ...
  const services = createServices(
    config,
    options.provider ?? new PriceChartingApiProvider(config.priceCharting),
  );
